...
int test_entryIsExpired(int argc, char **argv, int flags) {
    ...
    // Test with loading mode
    server.loading = 1;
    TEST_ASSERT(entryIsExpired(e1) == 0);
    TEST_ASSERT(entryIsExpired(e2) == 0);
    TEST_ASSERT(entryIsExpired(e3) == 0);
    TEST_ASSERT(entryIsExpired(e4) == 0);
    server.loading = 0;
    
    // Test with import mode and import source client
    struct client client_instance;
    server.import_mode = 1;
    server.current_client = &client_instance;
    server.current_client->flag.import_source = 1;
    TEST_ASSERT(entryIsExpired(e1) == 0);
    TEST_ASSERT(entryIsExpired(e2) == 0);
    TEST_ASSERT(entryIsExpired(e3) == 0);
    TEST_ASSERT(entryIsExpired(e4) == 0);
    
    // Test with import mode but not import source client
    server.current_client->flag.import_source = 0;
    TEST_ASSERT(entryIsExpired(e1) == 0);
    TEST_ASSERT(entryIsExpired(e2) == 0);
    TEST_ASSERT(entryIsExpired(e3) == 0);
    TEST_ASSERT(entryIsExpired(e4) == 1);
